...
if [ ! "$KSU" ]; then
    ...
    if echo "$MAGISK_VER" | grep -qE "kitsune|delta"; then
        ui_print "*********************************************************"
    ...
    if [ -z "$KSU_KERNEL_VER_CODE" ] || [ "$KSU_KERNEL_VER_CODE" -lt "$min_ksu_version" ] || [ "$KSU_VER_CODE" -lt "$min_ksud_version" ]; then
        ui_print "*********************************************************"
...
if [ ! -f "${clash_data_dir}/clashkernel/clashMeta" ]; then
    ...
    elif [ -f "${MODPATH}/bin/clashMeta-android-default.tar.bz2" ]; then
        tar -xjf "${MODPATH}/bin/clashMeta-android-default.tar.bz2" -C "${clash_data_dir}/clashkernel/"
        mv -f "${clash_data_dir}/clashkernel/clashMeta-android-default" "${clash_data_dir}/clashkernel/clashMeta"
    else
        ui_print "未找到架构: ${ABI}"
        abort "请使用 “make default” 为 ${ABI} 架构编译 clashMeta"
    fi
fi
...
if [ -f "${clash_data_dir}/clash.config" ]; then
    ...
    old_version="$(grep -i "version" "${clash_data_dir}/clash.config" | awk -F '=' '{print $2}' | sed 's/\"//g')"
    new_version="$(grep -i "version" "${MODPATH}/clash/clash.config" | awk -F '=' '{print $2}' | sed 's/\"//g')"
    ...
    if [ -n "$old_version" ] && [ "$old_version" = "$new_version" ]; then
        ...
        cp -vf "${clash_data_dir}/clash.config" "${clash_data_dir}/clash.config.old"
    fi
fi

# 安装 Dashboard APK（如未安装）
if ! pm list packages | grep -q "com.dashboard.kotlin"; then
...
如何使用 mihomo 以及配置文件文档请查阅→https://wiki.metacubex.one/config
预设配置文件在 /data/clash/config.yaml.example
请重命名为 config.yaml 后使用 DashBoard 启动/停止
或使用 tools 文件夹下的 start.sh/stop.sh
************************************************
